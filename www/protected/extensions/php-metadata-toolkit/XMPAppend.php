...
function get_xmp_dc( $XMP_array ) {
  // NOTE: Called caption.
  $outputarray = array("caption" => "");
  ...
  if ( $dublincore_block != FALSE ) {
    ...
    if ( $Item != FALSE ) {
      ...
      if  ( ( array_key_exists( 'children', $Item ) ) &&
        ...
        $outputarray =
          add_to_field( $outputarray, 'caption' ,
    ...
    if ( $Item != FALSE ) {
      ...
      if  ( ( array_key_exists( 'children', $Item ) ) &&
            ...
            ( array_key_exists( 'value', $Item['children'][0]['children'][0] ) ) ) {
        $outputarray =
          add_to_field( $outputarray, 'author' ,
                        HTML_UTF8_Escape( $Item['children'][0]['children'][0]['value'] ),
                        "\n" );
      }
    }
    ...
    if ( $Item != FALSE ) {
      ...
      if  ( ( array_key_exists( 'children', $Item ) ) &&
            ...
            ( array_key_exists( 'value', $Item['children'][0]['children'][0] ) ) ) {
        $outputarray =
          add_to_field( $outputarray, 'title' ,
                        HTML_UTF8_Escape( $Item['children'][0]['children'][0]['value'] ), "," );
      }
    }
    ...
    if ( $Item != FALSE ) {
      ...
      if  ( ( array_key_exists( 'children', $Item ) ) &&
            ...
            ( array_key_exists( 'value', $Item['children'][0]['children'][0] ) ) ) {
        $outputarray =
          add_to_field( $outputarray, 'copyrightnotice' ,
                        HTML_UTF8_Escape( $Item['children'][0]['children'][0]['value'] ),
                        "," );
      }
    }
 ...
 *
 * Description:  Retrieves XMP Dublin Core (dc) fields, appends passed-in data
 *               to them and then returns the combined values.
 *
 *               (NOTE: For now we only care about the dc:description field,
 *                and that is the *ONLY* field that we are appending to,
 *                however we can/will expand the list in the future)
 *
 * Parameters:   XMP_array - An array containing the existing XMP information
 *                 as retrieved by read_XMP_array_from_text.
 *                 (saves having to parse the XMP again)
 *
...
function append_to_xmp_dc( $XMP_array, $dc ) {
  // This returns a key-value pair array of Dublin Core fields.
  $existing_xmp_dc = get_xmp_dc($XMP_array);
  ...
  if ($XMP_array == FALSE ) {
    ...
    $new_XMP_array = XMP_Check( $GLOBALS[ 'Blank XMP Structure' ],
                                $XMP_array );
  }
  ...
  $new_value = $existing_xmp_dc["caption"] . "\n" . $dc["description"];
