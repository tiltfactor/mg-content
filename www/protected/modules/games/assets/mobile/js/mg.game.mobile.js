  ...
  return $.extend(MG_GAME_API, {
    ...
    init : function (options) {
      var settings = $.extend(options, {
        ongameinit: MG_GAME_MOBILE.ongameinit
      });
...
// mechanics are close enough to just work without much tweaking!
      MG_GAME_MOBILE.wordField = $("#words");
      
      // submit on enter
      MG_GAME_MOBILE.wordField.focus().keydown(function(event) {
        if(event.keyCode == 13) {
          MG_GAME_MOBILE.onsubmit(); 
          return false;
        }
      });
      
      MG_GAME_MOBILE.submitButton = $("#button-play").click(MG_GAME_MOBILE.onsubmit);
    // TRY to get pass button to submit correct value.
    MG_GAME_MOBILE.passButton = $("#button-pass").click(MG_GAME_MOBILE.onpass);
...
// Delete the default footer content.
    ...
    renderTurn : function (response, score_info, turn_info, licence_info, more_info) {
      ...
      if (!MG_GAME_MOBILE.game.user_authenticated) {
      ...
      $("#stage").fadeIn(1000, function () {MG_GAME_MOBILE.busy = false;MG_GAME_MOBILE.wordField.focus();});
    },
...
// We should hide the input_area as well.
      ...
      if (MG_GAME_MOBILE.game.play_once_and_move_on == 1) {
        ...
        $("#box1").hide();
        window.setTimeout(function() {window.location = score_info.play_once_and_move_on_url;}, MG_GAME_MOBILE.playOnceMoveOnFinalScreenWaitingTime);
        
        var updateRemainingTime = function () {
          MG_GAME_MOBILE.playOnceMoveOnFinalScreenWaitingTime -= 1000;
          if (MG_GAME_MOBILE.playOnceMoveOnFinalScreenWaitingTime >= 1) {
            $('#remainingTime').text(MG_GAME_MOBILE.playOnceMoveOnFinalScreenWaitingTime/1000);
	...
	//console.log("appending final summary");
      ...
      //console.log("Adding play-again button");
    ...
    onresponse : function (response) {
      ...
      MG_GAME_MOBILE.turn++;
      MG_GAME_MOBILE.turns.push(response.turn);
      
      //var more_info = {url: 'http://mg.com', name: 'foo'};
      if ($.trim(MG_GAME_MOBILE.game.more_info_url) != "")
          var more_info = {url: MG_GAME_MOBILE.game.more_info_url, name: MG_GAME_MOBILE.game.name};
      
      if (MG_GAME_MOBILE.turn > MG_GAME_MOBILE.game.turns) { // render final result
        var licence_info = [];  
        ...
        if (MG_GAME_MOBILE.turns.length) { // extract scoring and licence info
        ...
        //score box
        var score_info = {
          user_name : MG_GAME_MOBILE.game.user_name,
          user_score : MG_GAME_MOBILE.game.user_score,
          current_score : response.turn.score,
          user_num_played : MG_GAME_MOBILE.game.user_num_played,
          turns : MG_GAME_MOBILE.game.turns,
          current_turn : MG_GAME_MOBILE.turn,
        ...
        if (MG_GAME_MOBILE.game.play_once_and_move_on == 1) {
          if (MG_GAME_MOBILE.game.play_once_and_move_on_url == "")
            MG_GAME_MOBILE.game.play_once_and_move_on_url = "/";
     ...
     */
    onsubmit : function () {
      if (!MG_GAME_MOBILE.busy) {
        var tags = $.trim(MG_GAME_MOBILE.wordField.val());
        if (tags == "") {
          // val filtered for all white spaces (trim)
          MG_GAME_MOBILE.error("<h1>Ooops</h1><p>Please enter at least one word</p>");
        } else {
          
          // text entered 
          MG_GAME_API.curtain.show();
          MG_GAME_MOBILE.busy = true;
    ...
    onpass : function () {
      if (!MG_GAME_MOBILE.busy) {
        var tags = MG_GAME_MOBILE.passString;
