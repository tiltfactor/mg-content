...
class ImportController extends GxController {
  ...
  public function accessRules() {
  	return array(
  			...
  			array('allow', 
  				'actions'=>array('index', 'processimportedimages', 'uploadfromlocal', 'xuploadimage'),
  ...
  public function actionProcessImportedImages() {
    $this->layout='//layouts/column1';
  ...
  public function actionXUploadImage() {
    ...
    if (isset($model->file)) {
      ...
      if ($model->validate()) {
        ...
        while(file_exists($path.$model->name)) {
          $path_parts = pathinfo($model->name);
          $model->name = $path_parts['filename'] . "_" . $c . "." . $path_parts['extension'];
        }
        ...
        $info[] = array(
          ...
          'thumbnail_url' => 'http://metadatagames.test/uploads/image/'. $image->name,  // xxx make a thumbnail and serve the right sized preview
          'error' => null
        );
      } else {
        $info[] = array(
