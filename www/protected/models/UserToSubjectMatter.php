...
class UserToSubjectMatter_ extends BaseUserToSubjectMatter
{
  ...
  /**
   * List the subject matters values for the user filtered by medias. This method returns only subject matters
   * that belong to the media's collections
   * 
   * @param int $user_id The id of the user for whom the list should be generated
   * @param array $media_ids list of medias for which you would like to retrieve the related subject matters user values
   * @return array null if no values or array of arrays [[media.id, usm.subject_matter_id, usm.interest, usm.expertise, usm.trust], ... ]
   */
  public static function listForUserAndMedias($user_id, $media_ids) {
    $command = Yii::app()->db->createCommand()
                  ->select('i.id as media_id, usm.subject_matter_id, usm.interest, usm.expertise, usm.trust')
                  ->from('{{media}} i')
                  ->rightJoin('{{collection_to_media}} is2i', 'is2i.media_id=i.id')
                  ->rightJoin('{{collection_to_subject_matter}} is2sm', 'is2sm.collection_id=is2i.collection_id')
                  ->rightJoin('{{user_to_subject_matter}} usm', 'usm.subject_matter_id=is2sm.subject_matter_id')
                  ->where(array('and', 'usm.user_id=:userID', array(  'in', 'i.id', array_values($media_ids))), array('userID' => $user_id));
    $command->distinct = true;
    return $command->queryAll();
  }
  
  /**
   * List the subject matters MAX(values) for the user filtered by medias. This method returns only subject matters
   * that belong to the media's collections. It will only return one row per media.
   * 
   * @param int $user_id The id of the user for whom the list should be generated
   * @param array $media_ids list of medias for which you would like to retrieve the related subject matters user values
   * @return array null if no values or array of arrays [[media.id, usm.subject_matter_id, usm.interest, usm.expertise, usm.trust], ... ]
   */
  public static function listMAXForUserAndMedias($user_id, $media_ids) {
    return Yii::app()->db->createCommand()
                  ->select('i.id as media_id, MAX(usm.interest) as interest, MAX(usm.expertise) as expertise, MAX(usm.trust) as trust')
                  ->from('{{media}} i')
                  ->rightJoin('{{collection_to_media}} is2i', 'is2i.media_id=i.id')
                  ->rightJoin('{{collection_to_subject_matter}} is2sm', 'is2sm.collection_id=is2i.collection_id')
                  ->rightJoin('{{user_to_subject_matter}} usm', 'usm.subject_matter_id=is2sm.subject_matter_id')
